...
import {COURSES_PAGE_ADMIN_ROUTE, DEPARTMENT_EDIT_ADMIN_ROUTE} from "../../utils/consts.ts";
...
const UserEdit = () => {
    ...
    return (
        <div className="main-wrapper pb-5">
            ...
            <UserEditSidebar sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen}/>
            <AllCoursesModal editHeader={"Иванова"} setModalOpen={setModalOpen} modalOpen={modalOpen}/>
            ...
            <div className="flex flex-col gap-3.5">
                ...
                <hr/>
                <div className="flex gap-5">
                    <MySelect name="department" label="Отдел" items={DepartmentList} createText="отдел"
                              createLink={DEPARTMENT_EDIT_ADMIN_ROUTE} className="w-full"/>
                    <MySelect name="course" label="Прикрепить к курсу" items={courses} createText="курс"
                              createLink={COURSES_PAGE_ADMIN_ROUTE} className="w-full"/>
                </div>
                <h2>Прикреплённые курсы</h2>
                <div
                    className={classNames("flex gap-9 flex-wrap text-sm gap-y-3", courses?.length > 3 ? "justify-between" : "")}
                >
                    {courses.map(({id, name}) =>
                        <div
                            key={id}
                ...
                <div className="flex justify-between">
                    ...
                    <div className="flex gap-7 items-center">
                        ...
                        <button
                            className="bg-red-600 px-5 py-2 text-white rounded hover:bg-red-700 transition-colors"
                        >
                            Отмена
                        </button>
                    </div>
