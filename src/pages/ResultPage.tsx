...
export default function ResultPage() {
    ...
    const test = useAppSelector(state => selectSectionById(state, section?.testId));
    ...
    return (
        <div className="main-wrapper">
            <div className="flex justify-between items-center mb-20">
                <NavigationSteps/>
                <p>{section?.steps} / {section?.steps} шагов пройдено</p>
            </div>
            <div className="w-full flex-col justify-start items-center gap-12 flex">
                <div className="flex-col justify-start items-center gap-1.5 flex">
                    <h1>Результат</h1>
                    <p className="text-black text-lg font-semibold">Тест на тему: “{test?.name}”</p>
                </div>
                ...
                <div className="flex justify-center gap-7">
                    <Link to={COURSES_PAGE_ROUTE} className="px-5 py-2 bg-white rounded shadow-md hover:bg-gray-50 transition-colors">
                        Вернуться к курсу
                    </Link>
                    <Link to={SECTION_PAGE_ROUTE + "/" + (Number(sectionId) + 1)} className="px-5 py-2 bg-blue-700 rounded text-white hover:bg-blue-800 transition-colors">
                        Следующая тема
                    </Link>
                </div>
            </div>
