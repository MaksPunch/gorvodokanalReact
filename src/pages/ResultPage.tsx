    ...
    useEffect(() => {
        const rightAnswersCount = questions.reduce((count, question) => {
            if (question.type !== 'text') {
                const rightAnswers = answers.filter(el => el.questionId === question.id && el.rightAnswer && el.type !== 'text');
                const selectedAnswers = answers.filter(el => el.questionId === question.id && el.selected && el.type !== 'text');

                if (checkIfArraysEquals(rightAnswers, selectedAnswers)) {
                    return count + 1;
                }
            } else {
                const questionAnswer = answers.find(el => el.questionId === question.id && el.type === 'text');
