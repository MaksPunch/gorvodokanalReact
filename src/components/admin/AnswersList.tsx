...
const AnswersList = ({
  ...
  const questionAnswers = useAppSelector(
    selectAnswersByQuestionId(questionId, type),
  );
    ...
    <div className="flex flex-col gap-2.5 pl-7">
      {type === "text" && textAnswer ? (
        <input
          ...
          type="text"
          name={"question_text_" + questionId}
          id={"question_text_" + questionId}
          value={textAnswer.answer}
          onChange={(e) => changeName(textAnswer.id, e.target.value)}
        />
      ) : (
        questionAnswers.map(
          ({ answer, id, rightAnswer, type: answerType }) => {
