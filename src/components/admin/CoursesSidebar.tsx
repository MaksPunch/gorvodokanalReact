...
import {selectCourseById, setCourseName} from "../../store/reducers/courseSlice.ts";
import MyButton from "../MyButton.tsx";

const CoursesSidebar = ({courseId, setSidebarOpen, sidebarOpen}: {
    courseId: number,
    setSidebarOpen: ((arg1: boolean) => void),
    sidebarOpen: boolean,
}) => {
    const courseNameValue = useAppSelector(state => selectCourseById(state, courseId))?.name || "";
    ...
    return (
        <div ref={sideBarRef} className={classNames(
            ...
            <form action="#" method="POST" className={"flex flex-col gap-10"} onSubmit={(e) => e.preventDefault()}>
                <MyInput name={'course_name'} type={'text'} label={"Название курса"} value={name}
                         onChangeHandle={(e) => setName(e.target.value)}/>
                <div className="flex flex-col gap-7">
                    <div className="flex justify-between">
                        <h2>Темы в курсе</h2>
                        <Link to={SECTION_PAGE_ADMIN_ROUTE} className="text-blue-600 text-lg font-medium">
                            Создать тему
                        </Link>
                    </div>
                    <div className="flex flex-col gap-5">
                        {sections.map(({name, id }) =>
                            <div key={id} className="flex justify-between items-center">
                                <Link to={SECTION_PAGE_ADMIN_ROUTE + "/" + id} className="text-sm font-medium">{name}</Link>
                ...
                <div className="flex justify-between">
                    <MyButton className="text-sm" onClick={() => saveCourse()}>Сохранить</MyButton>
                    <button className="bg-red-600 px-5 py-2 text-sm text-white rounded hover:bg-red-700 transition-colors">Удалить курс</button>
                </div>
            </form>
